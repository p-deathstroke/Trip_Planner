...
const create = async(id, tripid, type, info) => {
    ...
    if(!type || typeof type != "string" || type.replace(/\s/g, '') == "") throw "type must be a valid string";
...
const read = async(userid, tripid, activityid) => {
    ...
    if(typeof userid != "string" || userid.replace(/\s/g, '') == "") throw "userid must be a valid string";
    ...
    if(typeof activityid != "string" || activityid.replace(/\s/g, '') == "") throw "activityid must be a valid string";
    if(!activityid.match(/^[0-9a-fA-F]{24}$/)) throw "id must be a valid ObjectId"; //https://stackoverflow.com/questions/13850819/can-i-determine-if-a-string-is-a-mongodb-objectid
...
const update = async(userid, tripid, activityid, info) => {
    ...
    let updatedInfo = await userList.updateOne({_id: ObjectId(userid)}, {$set: user});
    if(updatedInfo.modifiedCount === 0) throw "could not update book";
